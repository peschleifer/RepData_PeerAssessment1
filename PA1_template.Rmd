...
Make sure we have the needed packages loaded - this analysis requires dplyr, lubridate and ggplot2
```{r libs,results='hide',message=FALSE}
if ( !(require(dplyr) && require(lubridate) && require(ggplot2)) ) {
    stop ("You need to install the dplyr and ggplot2 packages to run this script")
}
```

Unzip and read the activity data file. Convert the date column into a date. Keep the interval column as a number, but add a date representaiton
```{r}
actMonRaw <- read.csv( unz('activity.zip','activity.csv'))
actMon <- tbl_df(actMonRaw) %>%
    mutate( date = as.Date(date) ) 
actMon$intervalTime=strptime( sprintf("%04d",actMon$interval),  format="%H%M")
```
...
Then we will smooth the remaining missing data with the mean of the readings from the nearest days before and day after that have actual readings
...
Now make a histogram with the imputed data included and compare the mean and median with the original results
